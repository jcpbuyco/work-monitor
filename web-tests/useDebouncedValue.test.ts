...
import { useDebouncedValue } from "../src/web/useDebouncedValue.ts";
...
describe("useDebouncedValue", () => {
  ...
  it("only adopts a new value after the delay elapses with no further change", () => {
    vi.useFakeTimers();
  ...
  it("resets the timer on every change, so only the LAST value in a burst ever lands", () => {
    vi.useFakeTimers();
    const { result, rerender } = renderHook(({ v }) => useDebouncedValue(v, 300), { initialProps: { v: "a" } });
    rerender({ v: "ab" });
