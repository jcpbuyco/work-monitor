...
import { repoNameFromGitDir, resolveProjectName } from "../src/server/resolve-project.ts";
...
describe("resolveProjectName", () => {
  ...
  it("resolves a worktree's cwd to the repo name, not the branch dir", async () => {
    expect(await resolveProjectName(worktree)).toBe("myproj");
  });
  it("resolves a normal repo cwd to the repo name", async () => {
    expect(await resolveProjectName(repo)).toBe("myproj");
  });
  it("falls back to the path basename for a non-git path", async () => {
    expect(await resolveProjectName("/no/such/dir/foobar")).toBe("foobar");
  });
});
